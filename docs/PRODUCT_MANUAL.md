...
> 版本：1.0.0 | 更新日期：2026-06-08 | 维护者：jw
...
> Windows 环境下提供对应的 `.bat` 脚本（start.bat / stop.bat / status.bat）。
...
|--------|------|------|------|
| 0 | 成功 | 请求处理成功 | — |
| -1 | 业务异常 | 业务逻辑错误（如数据不存在、重复等） | `BusinessException` |
| 400 | 参数错误 | 请求参数校验不通过 | `PARAM_ERROR` |
| 401 | 未授权 | 需要身份认证（预留） | `UNAUTHORIZED` |
| 403 | 禁止访问 | 无权限（预留） | `FORBIDDEN` |
| 404 | 资源不存在 | 查询的数据不存在（预留） | `NOT_FOUND` |
